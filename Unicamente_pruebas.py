...
def main()->None:
    ...
    work_a = executor.submit(Black_Hole_Geodesic, -L_screen/2,
                        0, L_screen/2, L_screen/4, N_pix_x, N_pix_y, "a")
    work_b = executor.submit(Black_Hole_Geodesic, 0, L_screen/2,
                        L_screen/2, L_screen/4, N_pix_x, N_pix_y, "b")
    work_c = executor.submit(Black_Hole_Geodesic, -L_screen/2, 0,
                        L_screen/4, 0, N_pix_x, N_pix_y, "c")
    work_d = executor.submit(Black_Hole_Geodesic, 0, L_screen/2,
                        L_screen/4, 0, N_pix_x, N_pix_y, "d")
    work_e = executor.submit(Black_Hole_Geodesic, -L_screen/2, 0, 
                        0, -L_screen/4, N_pix_x, N_pix_y, "e")
    work_f = executor.submit(Black_Hole_Geodesic, 0, L_screen/2,
                        0, -L_screen/4, N_pix_x, N_pix_y, "f")
    work_g = executor.submit(Black_Hole_Geodesic, -L_screen/2, 0,
                        -L_screen/4, -L_screen/2, N_pix_x, N_pix_y, "g")
    work_h = executor.submit(Black_Hole_Geodesic, 0, L_screen/2,
                        -L_screen/4, -L_screen/2, N_pix_x, N_pix_y, "h") 
    ...
    work_a = executor.submit(Black_Hole_Geodesic, -L_screen/2,
                        -L_screen/4, L_screen/2, -L_screen/2, N_pix_x, 3*N_pix_y, "a")
    work_b = executor.submit(Black_Hole_Geodesic, -L_screen/4, 0,
                        L_screen/2, L_screen/6, N_pix_x, N_pix_y, "b")
    work_c = executor.submit(Black_Hole_Geodesic, 0, L_screen/4,
                        L_screen/2, L_screen/6, N_pix_x, N_pix_y, "c")
    work_d = executor.submit(Black_Hole_Geodesic, -L_screen/4, 0,
                        L_screen/6, -L_screen/6, N_pix_x, N_pix_y, "d")
    work_e = executor.submit(Black_Hole_Geodesic, 0, L_screen/4, 
                        L_screen/6, -L_screen/6, N_pix_x, N_pix_y, "e")
    work_f = executor.submit(Black_Hole_Geodesic, -L_screen/4, 0,
                        -L_screen/6, -L_screen/2, N_pix_x, N_pix_y, "f")
    work_g = executor.submit(Black_Hole_Geodesic, 0, L_screen/4,
                        -L_screen/6, -L_screen/2, N_pix_x, N_pix_y, "g")
    work_h = executor.submit(Black_Hole_Geodesic, L_screen/4, L_screen/2,
                        L_screen/2, -L_screen/2, N_pix_x, 3*N_pix_y, "h") 
    ...
    for j in range(N_pix_y):
        for i in range(N_pix_x):
            csv_Color.writerow(work_a.result()[0][j+N_pix_y][i])
            csv_Position.writerow(work_a.result()[1][j+N_pix_y][i])
        for i in range(N_pix_x):
            csv_Color.writerow(work_d.result()[0][j][i])
            csv_Position.writerow(work_d.result()[1][j][i])
        for i in range(N_pix_x):
            ...
            csv_Position.writerow(work_e.result()[1][j][i])
        for i in range(N_pix_x):
            csv_Color.writerow(work_h.result()[0][j+N_pix_y][i])
            csv_Position.writerow(work_h.result()[1][j+N_pix_y][i])

    for j in range(N_pix_y):
        for i in range(N_pix_x):
            csv_Color.writerow(work_a.result()[0][j+2*N_pix_y][i])
            csv_Position.writerow(work_a.result()[1][j+2*N_pix_y][i])
        for i in range(N_pix_x):
            csv_Color.writerow(work_f.result()[0][j][i])
            csv_Position.writerow(work_f.result()[1][j][i])
        for i in range(N_pix_x):
            ...
            csv_Position.writerow(work_g.result()[1][j][i])
        for i in range(N_pix_x):
            csv_Color.writerow(work_h.result()[0][j+2*N_pix_y][i])
            csv_Position.writerow(work_h.result()[1][j+2*N_pix_y][i]) """
