...
def Geodesic_Chris(Back_Im, N_pix ,t_0, r_0, phi_0, theta_0, p_t_0, p_r_0, p_phi_0, p_theta_0, M, a):
    ...
    momento_sub_0=[p_t_0, p_r_0, p_phi_0, p_theta_0] # Lista de los momentos iniciales con SUBindices
    for i in range(4):
    ...
    for i in range(N):
        ...
        h=Paso_adap(coord_act[5], coord_act[7] ,M) # Se elige el paso para cada iteracion
        ...
        for j in range(8):
            K2.append(Switch_punto(j, coord_ant[4]+(h/2)*K1[4], coord_ant[5]+(h/2)*K1[5], coord_ant[6]+(h/2)*K1[6], coord_ant[7]+(h/2)*K1[7],
            coord_ant[0]+(h/2)*K1[0], coord_ant[1]+(h/2)*K1[1], coord_ant[2]+(h/2)*K1[2], coord_ant[3]+(h/2)*K1[3], *param))
        for j in range(8):
            K3.append(Switch_punto(j, coord_ant[4]+(h/2)*K2[4], coord_ant[5]+(h/2)*K2[5], coord_ant[6]+(h/2)*K2[6], coord_ant[7]+(h/2)*K2[7],
            coord_ant[0]+(h/2)*K2[0], coord_ant[1]+(h/2)*K2[1], coord_ant[2]+(h/2)*K2[2], coord_ant[3]+(h/2)*K2[3], *param))
        for j in range(8):
            K4.append(Switch_punto(j, coord_ant[4]+(h)*K3[4], coord_ant[5]+(h)*K3[5], coord_ant[6]+(h)*K3[6], coord_ant[7]+(h)*K3[7],
            coord_ant[0]+(h)*K3[0], coord_ant[1]+(h)*K3[1], coord_ant[2]+(h)*K3[2], coord_ant[3]+(h)*K3[3], *param))
